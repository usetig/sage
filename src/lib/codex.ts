...
export function buildInitialPromptPayload(
  ...
  const promptText = [
    ...
    '- This read-only rule applies ONLY to you (Sage). Claude Code may run tools, write files, or make edits—do NOT treat Claude’s write actions as violations.',
    '',
    '# Task',
    ...
    'Use this field ONLY when verdict is "Concerns" or "Critical Issues" AND there\'s a meaningfully different approach that is better than Claude\'s approach.',
    '- Return empty string "" if verdict is "Approved"',
    '- Return empty string "" if the issue is about correctness/bugs (not approach)',
    '- Do NOT suggest alternatives just because you can think of other ways to do something',
    '',
    ...
    '- Do NOT flag Claude for writing files (e.g., plans under ~/.claude/) unless the user asked Claude not to write or the write is clearly harmful.',
