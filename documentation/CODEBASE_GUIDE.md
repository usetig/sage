...
- Sage does not override Codex settings; it inherits whatever is in `~/.codex/config.toml` (and `managed_config.toml` if present). Approval policy, sandbox mode, allowed MCP servers, and web access flags all carry through to Sage reviews.
...
- `sage config` creates/updates a dedicated `profiles.sage` entry with read-only defaults, RMCP enabled, web search allowed (`features.web_search_request=true`), and a Context7 MCP server so Sage can review with the same capabilities every run.
